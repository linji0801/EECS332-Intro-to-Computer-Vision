...
def FindCur(contour, l, threshold_l, threshold_h):
    ...
    Centroid = np.mean(Contour[CosTheta > 0], axis=0)
...
def FingerTipDir(list_FingerTipHisto, PreDirection):
    if len(list_FingerTipHisto):
        ...
        index = np.where(Histo == np.max(Histo))
        Histo_temp = copy.deepcopy(Histo)
        Histo_temp[index[0]] = 0
        if len(index[0]) > 1 or np.max(Histo) - np.max(Histo_temp) < 8:  # more than one fingertip direction
            FingerTipDir = PreDirection
        elif np.max(Histo) > 0.5 * np.sum(Histo):  # obvious fingertip detected
            theta = index[0][0] * 30
...
while True:
    ...
    if ret == True:
        ...
        hand_binary = \
        cv2.threshold(HandMov1[:, :, 0] + HandMov1[:, :, 1] + HandMov1[:, :, 2], 127, 255, cv2.THRESH_BINARY)[1]
        image, cnts, hierarchy = cv2.findContours(hand_binary.astype(np.uint8), cv2.RETR_EXTERNAL,
        ...
        if MovFlag:  # have detected motion
            ...
            HandMov2[y1:y2, x1:x2, :] = BGR.Segment(cv2.GaussianBlur(frame[y1:y2, x1:x2, :], (11, 11), 1), parameter,
                                                    threshold_h)
            cv2.imshow('hand', HandMov2 / 255)

            hand_binary = \
            cv2.threshold(HandMov2[:, :, 0] + HandMov2[:, :, 1] + HandMov2[:, :, 2], 127, 255, cv2.THRESH_BINARY)[1]
            ...
            if temp:  # find fingertip candidates -> try to find fingertip direction and gesture
                [FingerTipCands, Centroid] = FindCur(LargestHull, 15, 0.3, 0.93)  # 20 0.3 0.94 #0.4 0.95
            ...
            # find fingertip
            FingerTip = FindFingerTip(np.squeeze(hulls), [(x1 + x2) / 2, (y1 + y2) / 2],
                                      Direction)  # find finger from all hulls
